...
int main(int argc, char *argv[] ) {
    ...
    /* Allocate memory to the in buffer for possible receiving */
    ...
    /* Attempt to read data from server, if any */
    bytes_read = recv ( sockfd, (void *) inbuf, BUF_SIZE, 0 );
    if (bytes_read < 0) {
        fprintf(stderr, "ERROR: Failed to receive data from the server.\n");
        close(sockfd);
        exit(EXIT_FAILURE);
        ...
        /* Process received data stored in `inbuf` */
